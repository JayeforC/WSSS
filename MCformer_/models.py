...
class MCTformerV2(VisionTransformer):
    ...
    def forward(self, x, return_att=False, n_layers=12, attention_type='fused'):
        ...
        """
            attn_weights: attn maps among 12 attn layers [12,B,N,N]
            feature_map : output of last attn layers and then add conv layer to generate feature map [B,C,h,w]
            mtatt       : Amct in the paper . [B,C,h,w]
            cams(fused) : A in the paper    
            x_cls_logits: class_token_scores
        ...
        x_patch = x_patch.permute([0, 3, 1, 2]) # B,D,H,W
        x_patch = x_patch.contiguous()
        x_patch = self.head(x_patch) # B,C,H,W    num of class
        x_patch_logits = self.avgpool(x_patch).squeeze(3).squeeze(2) # B, C, 1,1,1,1
        ...
        mtatt = attn_weights[-n_layers:].sum(0)[:, 0:self.num_classes, self.num_classes:].reshape([n, c, h, w]) # Ac2p in the paper
