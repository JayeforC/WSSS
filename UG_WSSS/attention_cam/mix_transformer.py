...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        #attn_ = F.avg_pool2d(attn_.permute(0,3,1,2), kernel_size=self.sr_ratio, stride=self.sr_ratio)
        attn_copy = attn_copy.reshape(-1, self.num_heads, attn.shape[-1], attn.shape[-1])  ##B，num_heads, N,N
        #####
...
class MixVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=1, num_classes=2, embed_dims=[64, 128, 256, 512],
